...
void compute_rhs(double t, const double* y, const double* field_params, int element_type, double* dydt, int dim) {
    ...
    // Dönen çerçeve düzeltmesi: Spin-Wheel base frekansını çıkar.
    // Omega[0,1]'den base presesyonu düşerek simülasyonu dönen referans
    // çerçevesinde çalıştırır; IQ demodülasyonuyla Δf ölçümünü kolaylaştırır.
...
void run_integration(double* y_init, const double* field_params,
    ...
    int     n_lat    = nFODO * 8;
    double* cod_x_sum = new double[n_lat]();
